...
upload_error($m){$de=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'Hochladen von Datei fehlgeschlagen.'.($de?" ".sprintf('Maximal erlaubte DateigrÃ¶sse ist %sB.',$de):""):'Datei existiert nicht.');}function
...
h($g->error);}function
check_sqlite_name($C){global$g;$yc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($yc)\$~",$C)){$g->error=sprintf('Bitte einen der Dateitypen %s benutzen.',str_replace("|",", ",$yc));return
false;}return
true;}function
...
set_schema($hg){global$g,$vh,$Eg;$J=$g->query("SET search_path TO ".idf_escape($hg));foreach(types()as$U){if(!isset($vh[$U])){$vh[$U]=0;$Eg['Benutzer-definierte Typen'][]=$U;}}return$J;}function
...
unconvert_field($n,$J){return$J;}function
support($Ac){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Ac);}$w="oracle";$vh=array();$Eg=array();foreach(array('Zahlen'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum oder Zeit'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Zeichenketten'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'BinÃ¤r'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$x=>$X){$vh+=$X;$Eg[$x]=array_keys($X);}$Bh=array();$Le=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Qc=array("length","lower","round","upper");$Vc=array("avg","count","count distinct","max","min","sum");$ac=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Sb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$tf=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Datenbank System<td>',html_select("auth[driver]",$Sb,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
...
selectLinks($Lg,$O=""){echo'<p class="links">';$Td=array("select"=>'Daten auswÃ¤hlen');if(support("table")||support("indexes"))$Td["table"]='Struktur anzeigen';if(support("table")){if(is_view($Lg))$Td["view"]='View Ã¤ndern';else$Td["create"]='Tabelle Ã¤ndern';}if($O!==null)$Td["edit"]='Neuer Datensatz';foreach($Td
...
selectQuery($H,$bh){global$w;return"<p><code class='jush-$w'>".h(str_replace("\n"," ",$H))."</code> <span class='time'>($bh)</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($H)."'>".'Ã„ndern'."</a>":"")."</p>";}function
...
selectOrderPrint($Qe,$f,$v){print_fieldset("sort",'Ordnen',$Qe);$r=0;foreach((array)$_GET["order"]as$x=>$X){if($X!=""){echo"<div>".select_input(" name='order[$r]' onchange='selectFieldChange(this.form);'",$f,$X),checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'absteigend')."</div>\n";$r++;}}echo"<div>".select_input(" name='order[$r]' onchange='selectAddRow(this);'",$f),checkbox("desc[$r]",1,false,'absteigend')."</div>\n","</div></fieldset>\n";}function
...
selectLengthPrint($ah){if($ah!==null){echo"<fieldset><legend>".'TextlÃ¤nge'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($ah)."'>","</div></fieldset>\n";}}function
...
messageQuery($H,$bh){global$w;restart_session();$ad=&get_session("queries");$s="sql-".count($ad[$_GET["db"]]);if(strlen($H)>1e6)$H=preg_replace('~[\x80-\xFF]+$~','',substr($H,0,1e6))."\n...";$ad[$_GET["db"]][]=array($H,time(),$bh);return" <span class='time'>".@date("H:i:s")."</span> <a href='#$s' onclick=\"return !toggle('$s');\">".'SQL-Query'."</a>"."<div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H,1000).'</code></pre>'.($bh?" <span class='time'>($bh)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($ad[$_GET["db"]])-1)).'">'.'Ã„ndern'.'</a>':'').'</div>';}function
editFunctions($n){global$ac;$J=($n["null"]?"NULL/":"");foreach($ac
as$x=>$Qc){if(!$x||(!isset($_GET["call"])&&(isset($_GET["select"])||where($_GET)))){foreach($Qc
as$mf=>$X){if(!$mf||preg_match("~$mf~",$n["type"]))$J.="/$X";}if($x&&!preg_match('~set|blob|bytea|raw|file~',$n["type"]))$J.="/SQL";}}if($n["auto_increment"]&&!isset($_GET["select"])&&!where($_GET))$J='Auto-Inkrement';return
explode("/",$J);}function
...
';}$this->databasesPrint($oe);if(DB==""||!$oe){echo"<p class='links'>".(support("sql")?"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".'SQL-Query'."</a>\n<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".'Importieren'."</a>\n":"")."";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}if($_GET["ns"]!==""&&!$oe&&DB!=""){echo'<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Neue Tabelle erstellen'."</a>\n";if(!$S)echo"<p class='message'>".'Keine Tabellen.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($oe){global$b,$g;$j=$this->databases();echo'<form action="">
<p id="dbs">
';hidden_fields_get();$Eb=" onmousedown='dbMouseDown(event, this);' onchange='dbChange(this);'";echo"<span title='".'Datenbank'."'>DB</span>: ".($j?"<select name='db'$Eb>".optionlist(array(""=>"")+$j,DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".'Benutzung'."'".($j?" class='hidden'":"").">\n";if($oe!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".'Schema'.": <select name='ns'$Eb>".optionlist(array(""=>"")+$b->schemas(),$_GET["ns"])."</select>";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':(isset($_GET["privileges"])?'<input type="hidden" name="privileges" value="">':"")))),"</p></form>\n";}function
tablesPrint($S){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($S
as$Q=>$Bg){echo'<a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"]==$Q||$_GET["edit"]==$Q).">".'zeigen'."</a> ";$C=$this->tableName($Bg);echo(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])),(is_view($Bg)?"view":""))." title='".'Struktur anzeigen'."'>$C</a>":"<span>$C</span>")."<br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
as$vd=>$X){if($X[0]<$bh)unset($ud[$vd]);}}$td=&$ud[$b->bruteForceKey()];if(!$td)$td=array($bh+30*60,0);$td[1]++;$qg=serialize($ud);rewind($Nc);fwrite($Nc,$qg);ftruncate($Nc,strlen($qg));flock($Nc,LOCK_UN);fclose($Nc);}$Ia=$_POST["auth"];if($Ia){$ud=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$td=$ud[$b->bruteForceKey()];$xe=($td[1]>30?$td[0]-time():0);if($xe>0)auth_error(sprintf('Too many unsuccessful logins, try again in %d minute(s).',ceil($xe/60)));session_regenerate_id();$l=$Ia["driver"];$N=$Ia["server"];$V=$Ia["username"];$G=(string)$Ia["password"];$k=$Ia["db"];set_password($l,$N,$V,$G);$_SESSION["db"][$l][$N][$V][$k]=true;if($Ia["permanent"]){$x=base64_encode($l)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($k);$zf=$b->permanentLogin(true);$nf[$x]="$x:".base64_encode($zf?encrypt_string($G,$zf):"");cookie("adminer_permanent",implode(" ",$nf));}if(count($_POST)==1||DRIVER!=$l||SERVER!=$N||$_GET["username"]!==$V||DB!=$k)redirect(auth_url($l,$N,$V,$k));}elseif($_POST["logout"]){if($Zc&&!verify_token()){page_header('Abmelden','CSRF Token ungÃ¼ltig. Bitte die Formulardaten erneut abschicken.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Abmeldung erfolgreich.');}}elseif($nf&&!$_SESSION["pwds"]){session_regenerate_id();$zf=$b->permanentLogin();foreach($nf
...
auth_error($m){global$b,$Zc;$tg=session_name();if(!$_COOKIE[$tg]&&$_GET[$tg]&&ini_bool("session.use_only_cookies"))$m='Sitzungen mÃ¼ssen aktiviert sein.';elseif(isset($_GET["username"])){if(($_COOKIE[$tg]||$_GET[$tg])&&!$Zc)$m='Sitzungsdauer abgelaufen, bitte erneut anmelden.';else{add_invalid_login();$G=get_password();if($G!==null){if($G===false)$m.='<br>'.sprintf('Master password expired. <a href="http://www.adminer.org/en/extension/" target="_blank">Implement</a> %s method to make it permanent.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header('Login',$m,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Keine Erweiterungen installiert',sprintf('Keine der unterstÃ¼tzten PHP-Erweiterungen (%s) ist vorhanden.',implode(", ",$tf)),false);page_footer("auth");exit;}$g=connect();}$l=new
Min_Driver($g);if(!is_object($g)||!$b->login($_GET["username"],get_password()))auth_error((is_string($g)?$g:'UngÃ¼ltige Anmelde-Informationen.'));if($Ia&&$_POST["token"])$_POST["token"]=$T;$m='';if($_POST){if(!verify_token()){$od="max_input_vars";$ee=ini_get($od);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$x){$X=ini_get($x);if($X&&(!$ee||$X<$ee)){$od=$x;$ee=$X;}}}$m=(!$_POST["token"]&&$ee?sprintf('Die maximal erlaubte Anzahl der Felder ist Ã¼berschritten. Bitte %s erhÃ¶hen.',"'$od'"):'CSRF Token ungÃ¼ltig. Bitte die Formulardaten erneut abschicken.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$m=sprintf('POST data zu gross. Reduzieren Sie die GrÃ¶sse oder vergrÃ¶ssern Sie den Wert %s in der Konfiguration.',"'post_max_size'");if(isset($_GET["sql"]))$m.=' '.'You can upload a big SQL file via FTP and import it from server.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
...
edit_type($x,$n,$jb,$Jc=array()){global$Eg,$vh,$Bh,$He;$U=$n["type"];echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),'>';if($U&&!isset($vh[$U])&&!isset($Jc[$U]))array_unshift($Eg,$U);if($Jc)$Eg['FremdschlÃ¼ssel']=$Jc;echo
optionlist($Eg,$U),'</select>
<td><input name="',$x,'[length]" value="',h($n["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$n["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();"><td class="options">';echo"<select name='$x"."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($jb,$n["collation"]).'</select>',($Bh?"<select name='$x"."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($Bh,$n["unsigned"]).'</select>':''),(isset($n['on_update'])?"<select name='$x"."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".'ON UPDATE'.")","CURRENT_TIMESTAMP"),$n["on_update"]).'</select>':''),($Jc?"<select name='$x"."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$He),$n["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($ob?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.1.0' alt='+' title='".'HinzufÃ¼gen'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
...
',(support("comment")?"<td".($ob?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($n["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.1.0' alt='+' title='".'HinzufÃ¼gen'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.1.0' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.1.0' alt='v' title='".'Nach unten'."'>&nbsp;":""),($Ue==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.1.0' alt='x' title='".'Entfernen'."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
connect_error(){global$b,$g,$T,$m,$Sb;if(DB!=""){header("HTTP/1.1 404 Not Found");page_header('Datenbank'.": ".h(DB),'Datenbank ungÃ¼ltig.',true);}else{if($_POST["db"]&&!$m)queries_redirect(substr(ME,0,-1),'Datenbanken entfernt.',drop_databases($_POST["db"]));page_header('Datenbank auswÃ¤hlen',$m,false);echo"<p class='links'>\n";foreach(array('database'=>'Neue Datenbank','privileges'=>'Rechte','processlist'=>'Prozessliste','variables'=>'Variablen','status'=>'Status',)as$x=>$X){if(support($x))echo"<a href='".h(ME)."$x='>$X</a>\n";}echo"<p>".sprintf('Version %s: %s, mit PHP-Erweiterung %s',$Sb[DRIVER],"<b>".h($g->server_info)."</b>","<b>$g->extension</b>")."\n","<p>".sprintf('Angemeldet als: %s',"<b>".h(logged_user())."</b>")."\n";$j=$b->databases();if($j){$ig=support("scheme");$jb=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n","<thead><tr>".(support("database")?"<td>&nbsp;":"")."<th>".'Datenbank'." - <a href='".h(ME)."refresh=1'>".'Aktualisieren'."</a>"."<td>".'Collation'."<td>".'Tabellen'."<td>".'Size'." - <a href='".h(ME)."dbsize=1' onclick=\"return !ajaxSetHtml('".js_escape(ME)."script=connect');\">".'Compute'."</a>"."</thead>\n";$j=($_GET["dbsize"]?count_tables($j):array_flip($j));foreach($j
as$k=>$S){$ag=h(ME)."db=".urlencode($k);echo"<tr".odd().">".(support("database")?"<td>".checkbox("db[]",$k,in_array($k,(array)$_POST["db"])):""),"<th><a href='$ag'>".h($k)."</a>";$d=nbsp(db_collation($k,$jb));echo"<td>".(support("database")?"<a href='$ag".($ig?"&amp;ns=":"")."&amp;database=' title='".'Datenbank Ã¤ndern'."'>$d</a>":$d),"<td align='right'><a href='$ag&amp;schema=' id='tables-".h($k)."' title='".'Datenbankschema'."'>".($_GET["dbsize"]?$S:"?")."</a>","<td align='right' id='size-".h($k)."'>".($_GET["dbsize"]?db_size($k):"?"),"\n";}echo"</table>\n",(support("database")?"<fieldset><legend>".'Selected'." <span id='selected'></span></legend><div>\n"."<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^db/));\">\n"."<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n"."</div></fieldset>\n":""),"<script type='text/javascript'>tableCheck();</script>\n","<input type='hidden' name='token' value='$T'>\n","</form>\n";}}page_footer("db");}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(isset($_GET["import"]))$_GET["sql"]=$_GET["import"];if(!(DB!=""?$g->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect"||$_GET["script"]=="kill")){if(DB!=""||$_GET["refresh"]){restart_session();set_session("dbs",null);}connect_error();exit;}if(support("scheme")&&DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){header("HTTP/1.1 404 Not Found");page_header('Schema'.": ".h($_GET["ns"]),'Schema nicht gÃ¼ltig.',true);page_footer("ns");exit;}}$He="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";class
TmpFile{var$handler;var$size;function
TmpFile(){$this->handler=tmpfile();}function
write($tb){$this->size+=strlen($tb);fwrite($this->handler,$tb);}function
send(){fseek($this->handler,0);fpassthru($this->handler);fclose($this->handler);}}$kc="'(?:''|[^'\\\\]|\\\\.)*'";$pd="IN|OUT|INOUT";if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["callf"]))$_GET["call"]=$_GET["callf"];if(isset($_GET["function"]))$_GET["procedure"]=$_GET["function"];if(isset($_GET["download"])){$a=$_GET["download"];$o=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));$M=array(idf_escape($_GET["field"]));$I=$l->select($a,$M,array(where($_GET,$o)),$M);$K=($I?$I->fetch_row():array());echo$K[0];exit;}elseif(isset($_GET["table"])){$a=$_GET["table"];$o=fields($a);if(!$o)$m=error();$R=table_status1($a,true);page_header(($o&&is_view($R)?'View':'Tabelle').": ".h($a),$m);$b->selectLinks($R);$nb=$R["Comment"];if($nb!="")echo"<p>".'Kommentar'.": ".h($nb)."\n";if($o){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Spalte'."<td>".'Typ'.(support("comment")?"<td>".'Kommentar':"")."</thead>\n";foreach($o
as$n){echo"<tr".odd()."><th>".h($n["field"]),"<td title='".h($n["collation"])."'>".h($n["full_type"]).($n["null"]?" <i>NULL</i>":"").($n["auto_increment"]?" <i>".'Auto-Inkrement'."</i>":""),(isset($n["default"])?" [<b>".h($n["default"])."</b>]":""),(support("comment")?"<td>".nbsp($n["comment"]):""),"\n";}echo"</table>\n";}if(!is_view($R)){if(support("indexes")){echo"<h3 id='indexes'>".'Indizes'."</h3>\n";$v=indexes($a);if($v){echo"<table cellspacing='0'>\n";foreach($v
...
as$x=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'Ã„ndern'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Trigger hinzufÃ¼gen'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('Datenbankschema',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Og=array();$Pg=array();$C="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$C-".DB]?"$C-".DB:$C)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Yd,PREG_SET_ORDER);foreach($Yd
...
as$C=>$n){if(!isset($n["privileges"][$Ch?"update":"insert"])||$b->fieldName($n)=="")unset($o[$C]);}if($_POST&&!$m&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($Ch?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$v=indexes($a);$yh=unique_array($_GET["where"],$v);$Hf="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,'Datensatz gelÃ¶scht.',$l->delete($a,$Hf,!$yh));else{$O=array();foreach($o
as$C=>$n){$X=process_input($n);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($Ch){if(!$O)redirect($A);queries_redirect($A,'Datensatz geÃ¤ndert.',$l->update($a,$O,$Hf,!$yh));if(is_ajax()){page_headers();page_messages($m);exit;}}else{$I=$l->insert($a,$O);$Ld=($I?last_id():0);queries_redirect($A,sprintf('Datensatz%s hinzugefÃ¼gt.',($Ld?" $Ld":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($o
...
',checkbox("defaults",1,true,'Vorgabewerte festlegen',"columnShow(this.checked, 5)","jsonly");if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?"<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"".($ob?" checked":"").">".'Kommentar'."</label>".' <input name="Comment" id="Comment" value="'.h($K["Comment"]).'" maxlength="'.($g->server_info>=5.5?2048:60).'"'.($ob?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Speichern">
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if(support("partitioning")){$gf=preg_match('~RANGE|LIST~',$K["partition_by"]);print_fieldset("partition",'Partitionieren um',$K["partition_by"]);echo'<p>
...
Partitionen: <input type="number" name="partitions" class="size',($gf||!$K["partition_by"]?" hidden":""),'" value="',h($K["partitions"]),'">
...
<th>Name
<th><noscript><input type='image' class='icon' name='add[0]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.1.0' alt='+' title='HinzufÃ¼gen'></noscript>&nbsp;
</thead>
<?php
if($wf){echo"<tr><td>PRIMARY<td>";foreach($wf["columns"]as$x=>$e){echo
select_input(" disabled",$o,$e),"<label><input disabled type='checkbox'>".'absteigend'."</label> ";}echo"<td><td>\n";}$Ad=1;foreach($K["indexes"]as$u){if(!$_POST["drop_col"]||$Ad!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$Ad][type]",array(-1=>"")+$kd,$u["type"],($Ad==count($K["indexes"])?"indexesAddRow(this);":1)),"<td>";ksort($u["columns"]);$r=1;foreach($u["columns"]as$x=>$e){echo"<span>".select_input(" name='indexes[$Ad][columns][$r]' onchange=\"".($r==count($u["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".js_escape($w=="sql"?"":$_GET["indexes"]."_")."');\"",($o?array_combine($o,$o):$o),$e),($w=="sql"||$w=="mssql"?"<input type='number' name='indexes[$Ad][lengths][$r]' class='size' value='".h($u["lengths"][$x])."'>":""),($w!="sql"?checkbox("indexes[$Ad][descs][$r]",1,$u["descs"][$x],'absteigend'):"")," </span>";$r++;}echo"<td><input name='indexes[$Ad][name]' value='".h($u["name"])."' autocapitalize='off'>\n","<td><input type='image' class='icon' name='drop_col[$Ad]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.1.0' alt='x' title='".'Entfernen'."' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";}$Ad++;}echo'</table>
<p>
<input type="submit" value="Speichern">
<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["database"])){$K=$_POST;if($_POST&&!$m&&!isset($_POST["add_x"])){restart_session();$C=trim($K["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Datenbank entfernt.',drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($C),'Datenbank umbenannt.',rename_database($C,$K["collation"]));}else{$j=explode("\n",str_replace("\r","",$C));$Gg=true;$Kd="";foreach($j
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$K["collation"]))$Gg=false;$Kd=$k;}}queries_redirect(ME."db=".urlencode($Kd),'Datenbank erstellt.',$Gg);}}else{if(!$K["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),'Datenbank geÃ¤ndert.');}}page_header(DB!=""?'Datenbank Ã¤ndern':'Neue Datenbank',$m,array(),h(DB));$jb=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$jb);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$Rc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Rc,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($jb?html_select("collation",array(""=>"(".'Kollation'.")")+$jb,$K["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mssql'=>"ms187963.aspx",)):"");?>
<script type='text/javascript'>focus(document.getElementById('name'));</script>
<input type="submit" value="Speichern">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.1.0' alt='+' title='".'HinzufÃ¼gen'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["scheme"])){$K=$_POST;if($_POST&&!$m){$_=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$_,'Schema wurde gelÃ¶scht.');else{$C=trim($K["name"]);$_.=urlencode($C);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($C),$_,'Neues Schema erstellt.');elseif($_GET["ns"]!=$C)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($C),$_,'Schema geÃ¤ndert.');else
redirect($_);}}page_header($_GET["ns"]!=""?'Schema Ã¤ndern':'Neues Schema',$m);if(!$K)$K["name"]=$_GET["ns"];echo'
<form action="" method="post">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;if($_POST&&!$m){$C=trim($K["name"]);$Ea=" AS\n$K[select]";$A=ME."table=".urlencode($C);$ge='View geÃ¤ndert.';if(!$_POST["drop"]&&$a==$C&&$w!="sqlite")query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ea,$A,$ge);else{$Wg=$C."_adminer_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($C).$Ea,"DROP VIEW ".table($C),"CREATE VIEW ".table($Wg).$Ea,"DROP VIEW ".table($Wg),($_POST["drop"]?substr(ME,0,-1):$A),'View entfernt.',$ge,'View erstellt.',$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;if(!$m)$m=$g->error;}page_header(($a!=""?'View Ã¤ndern':'Neue View erstellen'),$m,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>Jede<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$sd,$K["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Cg,$K["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
...
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$bg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$K=$_POST;$K["fields"]=(array)$K["fields"];if($_POST&&!process_fields($K["fields"])&&!$m){$Wg="$K[name]_adminer_".uniqid();drop_create("DROP $bg ".idf_escape($da),create_routine($bg,$K),"DROP $bg ".idf_escape($K["name"]),create_routine($bg,array("name"=>$Wg)+$K),"DROP $bg ".idf_escape($Wg),substr(ME,0,-1),'Prozedur entfernt.','Prozedur geÃ¤ndert.','Prozedur erstellt.',$da,$K["name"]);}page_header(($da!=""?(isset($_GET["function"])?'Funktion Ã¤ndern':'Prozedur Ã¤ndern').": ".h($da):(isset($_GET["function"])?'Neue Funktion':'Neue Prozedur')),$m);if(!$_POST&&$da!=""){$K=routine($da,$bg);$K["name"]=$da;}$jb=get_vals("SHOW CHARACTER SET");sort($jb);$cg=routine_languages();echo'
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
',($cg?'Sprache'.": ".html_select("language",$cg,$K["language"]):""),'<input type="submit" value="Speichern">
<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$jb,$bg);if(isset($_GET["function"])){echo"<tr><td>".'Typ des RÃ¼ckgabewertes';edit_type("returns",$K["returns"],$jb);}echo'</table>
<p>';textarea("definition",$K["definition"]);echo'<p>
<input type="submit" value="Speichern">
';if($da!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$K=$_POST;if($_POST&&!$m){$_=substr(ME,0,-1);$C=trim($K["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$_,'Sequenz gelÃ¶scht.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($C),$_,'Neue Sequenz erstellt.');elseif($fa!=$C)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($C),$_,'Sequenz geÃ¤ndert.');else
redirect($_);}page_header($fa!=""?'Sequenz Ã¤ndern'.": ".h($fa):'Neue Sequenz',$m);if(!$K)$K["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($K["name"]),'" autocapitalize="off">
<input type="submit" value="Speichern">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$K=$_POST;if($_POST&&!$m){$_=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$_,'Typ gelÃ¶scht.');else
query_redirect("CREATE TYPE ".idf_escape(trim($K["name"]))." $K[as]",$_,'Typ erstellt.');}page_header($ga!=""?'Typ Ã¤ndern'.": ".h($ga):'Typ erstellen',$m);if(!$K)$K["as"]="AS ";echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$rh=trigger_options();$K=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$m&&in_array($_POST["Timing"],$rh["Timing"])&&in_array($_POST["Event"],$rh["Event"])&&in_array($_POST["Type"],$rh["Type"])){$Ge=" ON ".table($a);$Tb="DROP TRIGGER ".idf_escape($C).($w=="pgsql"?$Ge:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($Tb,$A,'Trigger entfernt.');else{if($C!="")queries($Tb);queries_redirect($A,($C!=""?'Trigger geÃ¤ndert.':'Trigger erstellt.'),queries(create_trigger($Ge,$_POST)));if($C!="")queries(create_trigger($Ge,$K+array("Type"=>reset($rh["Type"]))));}}$K=$_POST;}page_header(($C!=""?'Trigger Ã¤ndern'.": ".h($C):'Trigger hinzufÃ¼gen'),$m,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$rh["Timing"],$K["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>Ereignis<td>',html_select("Event",$rh["Event"],$K["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$rh["Event"])?" <input name='Of' value='".h($K["Of"])."' class='hidden'>":""),'<tr><th>Typ<td>',html_select("Type",$rh["Type"],$K["Type"]),'</table>
...
as$X){if($X[1]!="USAGE")$Sc["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$Sc["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$Ee=$B[1];}}if($_POST&&!$m){$Fe=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Fe",ME."privileges=",'Benutzer entfernt.');else{$ve=q($_POST["user"])."@".q($_POST["host"]);$jf=$_POST["pass"];if($jf!=''&&!$_POST["hashed"]){$jf=$g->result("SELECT PASSWORD(".q($jf).")");$m=!$jf;}$_b=false;if(!$m){if($Fe!=$ve){$_b=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $ve IDENTIFIED BY PASSWORD ".q($jf));$m=!$_b;}elseif($jf!=$Ee)queries("SET PASSWORD FOR $ve = ".q($jf));}if(!$m){$Yf=array();foreach($te
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".'Rechte'.doc_link(array('sql'=>"grant.html#priv_level"));$r=0;foreach($Sc
as$_e=>$Rc){echo'<th>'.($_e!="*.*"?"<input name='objects[$r]' value='".h($_e)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$ub=>$Lb){foreach((array)$Af[$ub]as$_f=>$nb){echo"<tr".odd()."><td".($Lb?">$Lb<td":" colspan='2'").' lang="en" title="'.h($nb).'">'.h($_f);$r=0;foreach($Sc
as$_e=>$Rc){$C="'grants[$r][".h(strtoupper($_f))."]'";$Y=$Rc[strtoupper($_f)];if($ub=="Server Admin"&&$_e!=(isset($Sc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".'Erlauben'."<option value='0'".($Y=="0"?" selected":"").">".'Verbieten'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$m){$Hd=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Hd++;}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$Hd),$Hd||!$_POST["kill"]);}page_header('Prozessliste',$m);echo'
<form action="" method="post">
...
';}$Kc=$b->dumpFormat();foreach((array)$_GET["columns"]as$e){if($e["fun"]){unset($Kc['sql']);break;}}if($Kc){print_fieldset("export",'Exportieren'." <span id='selected2'></span>");$af=$b->dumpOutput();echo($af?html_select("output",$af,$wa["output"])." ":""),html_select("format",$Kc,$wa["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}echo(!$Tc&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");}if($b->selectImportPrint()){print_fieldset("import",'Importieren',!$L);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$wa["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($gc,'strlen'),$f);echo"<p><input type='hidden' name='token' value='$T'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$Bg=isset($_GET["status"]);page_header($Bg?'Status':'Variablen');$Mh=($Bg?show_status():show_variables());if(!$Mh)echo"<p class='message'>".'Keine Daten.'."\n";else{echo"<table cellspacing='0'>\n";foreach($Mh
...
as$x=>$X)json_row("sum-$x",format_number($X));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$k=>$X){json_row("tables-$k",$X);json_row("size-$k",db_size($k));}json_row("");}exit;}else{$Sg=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Sg&&!$m&&!$_POST["search"]){$I=true;$ge="";if($w=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$I=truncate_tables($_POST["tables"]);$ge='Tabellen sind entleert worden (truncate).';}elseif($_POST["move"]){$I=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ge='Tabellen verschoben.';}elseif($_POST["copy"]){$I=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ge='Tabellen wurden kopiert.';}elseif($_POST["drop"]){if($_POST["views"])$I=drop_views($_POST["views"]);if($I&&$_POST["tables"])$I=drop_tables($_POST["tables"]);$ge='Tabellen wurden entfernt (drop).';}elseif($w!="sql"){$I=($w=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$ge='Tables have been optimized.';}elseif(!$_POST["tables"])$ge='Keine Tabellen.';elseif($I=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($K=$I->fetch_assoc())$ge.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$ge,$I);}page_header(($_GET["ns"]==""?'Datenbank'.": ".h(DB):'Schema'.": ".h($_GET["ns"])),$m,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".'Tabellen und Views'."</h3>\n";$Rg=tables_list();if(!$Rg)echo"<p class='message'>".'Keine Tabellen.'."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".'Suche in Tabellen'." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Suchen'."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabelle','<td>'.'Motor','<td>'.'Collation','<td>'.'DatengrÃ¶sse','<td>'.'IndexgrÃ¶sse','<td>'.'Freier Bereich','<td>'.'Auto-Inkrement','<td>'.'DatensÃ¤tze',(support("comment")?'<td>'.'Kommentar':''),"</thead>\n";$S=0;foreach($Rg
as$C=>$U){$Ph=($U!==null&&!preg_match('~table~i',$U));echo'<tr'.odd().'><td>'.checkbox(($Ph?"views[]":"tables[]"),$C,in_array($C,$Sg,true),"","formUncheck('check-all');"),'<th>'.(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($C).'" title="'.'Struktur anzeigen'.'">'.h($C).'</a>':h($C));if($Ph){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.'View Ã¤ndern'.'">'.'View'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.'Daten auswÃ¤hlen'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Tabelle Ã¤ndern'),"Index_length"=>array("indexes",'Indizes Ã¤ndern'),"Data_free"=>array("edit",'Neuer Datensatz'),"Auto_increment"=>array("auto_increment=1&create",'Tabelle Ã¤ndern'),"Rows"=>array("select",'Daten auswÃ¤hlen'),)as$x=>$_){$s=" id='$x-".h($C)."'";echo($_?"<td align='right'>".(support("table")||$x=="Rows"||(support("indexes")&&$x!="Data_length")?"<a href='".h(ME."$_[0]=").urlencode($C)."'$s title='$_[1]'>?</a>":"<span$s>?</span>"):"<td id='$x-".h($C)."'>&nbsp;");}$S++;}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d insgesamt',count($Rg)),"<td>".nbsp($w=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x)echo"<td align='right' id='sum-$x'>&nbsp;";echo"</table>\n";if(!information_schema(DB)){$Jh="<input type='submit' value='".'Vacuum'."'".on_help("'VACUUM'")."> ";$Me="<input type='submit' name='optimize' value='".'Optimieren'."'".on_help($w=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'")."> ";echo"<fieldset><legend>".'Selected'." <span id='selected'></span></legend><div>".($w=="sqlite"?$Jh:($w=="pgsql"?$Jh.$Me:($w=="sql"?"<input type='submit' value='".'Analysieren'."'".on_help("'ANALYZE TABLE'")."> ".$Me."<input type='submit' name='check' value='".'PrÃ¼fen'."'".on_help("'CHECK TABLE'")."> "."<input type='submit' name='repair' value='".'Reparieren'."'".on_help("'REPAIR TABLE'")."> ":"")))."<input type='submit' name='truncate' value='".'Entleeren (truncate)'."'".confirm().on_help($w=="sqlite"?"'DELETE'":"'TRUNCATE".($w=="pgsql"?"'":" TABLE'"))."> "."<input type='submit' name='drop' value='".'Entfernen'."'".confirm().on_help("'DROP TABLE'").">\n";$j=(support("scheme")?$b->schemas():$b->databases());if(count($j)!=1&&$w!="sqlite"){$k=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'In andere Datenbank verschieben'.": ",($j?html_select("target",$j,$k):'<input name="target" value="'.h($k).'" autocapitalize="off">')," <input type='submit' name='move' value='".'Verschieben'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopieren'."'>":""),"\n";}echo"<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^(tables|views)\[/));".(support("table")?" selectCount('selected2', formChecked(this, /^tables\[/) || $S);":"")."\">\n";echo"<input type='hidden' name='token' value='$T'>\n","</div></fieldset>\n";}echo"</form>\n","<script type='text/javascript'>tableCheck();</script>\n";}echo'<p class="links"><a href="'.h(ME).'create=">'.'Neue Tabelle erstellen'."</a>\n",(support("view")?'<a href="'.h(ME).'view=">'.'Neue View erstellen'."</a>\n":"");if(support("routine")){echo"<h3 id='routines'>".'Prozeduren'."</h3>\n";$dg=routines();if($dg){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Name'.'<td>'.'Typ'.'<td>'.'Typ des RÃ¼ckgabewertes'."<td>&nbsp;</thead>\n";odd('');foreach($dg
